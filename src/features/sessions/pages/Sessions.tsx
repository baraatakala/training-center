                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          ...
                          {session.requires_recording && (
                            <Badge variant="success">Ã°Å¸Å½Â¥ {formatRecordingVisibility(session.default_recording_visibility) || 'Recording'}</Badge>
                          )}
                          {session.feedback_enabled && (
                            <Badge variant="info">Ã°Å¸â€™Å“ {!isTeacher && !isAdmin ? 'After Check-In' : 'Feedback'}</Badge>
                          )}
                          {session.grace_period_minutes != null && session.grace_period_minutes > 0 && (
                            <Badge variant="default">Ã¢ÂÂ± {session.grace_period_minutes}m grace</Badge>
                          )}
                          {session.teacher_can_host === false && (
                            <Badge variant="warning">Ã°Å¸â€â€™ Student-hosted</Badge>
                          )}
                        </div>
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-500 border-t border-gray-100 dark:border-gray-700 pt-2">
                          {formatDate(session.start_date)} Ã¢â‚¬â€ {formatDate(session.end_date)}
                        </div>
                    ...
                    <TableRow key={session.session_id}>
                      <TableCell className="font-medium text-gray-900 dark:text-white min-w-[200px]">
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="flex flex-wrap gap-1 mt-0.5">
                            ...
                            {session.feedback_enabled && <Badge variant="info">Ã°Å¸â€™Å“ Feedback</Badge>}
                            {session.requires_recording && <Badge variant="success">Ã°Å¸Å½Â¥ Rec</Badge>}
                          </div>
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="hidden lg:table-cell">
                        <div className="flex flex-col text-xs text-gray-600 dark:text-gray-300">
                          ...
                          <span className="text-gray-400">Ã¢â€ â€™ {formatDate(session.end_date)}</span>
                        </div>
                      </TableCell>
                      <TableCell>
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-3">
              <p className="text-sm text-blue-800 dark:text-blue-300">
                <strong>Day changed:</strong> {dayChangeDialog.oldDay || 'None'} Ã¢â€ â€™ {dayChangeDialog.newDay || 'None'}
              </p>
              ...
              >
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  {dayChangeDialog.lastAttendedDate
                    ...
                    : 'No attendance records found Ã¢â‚¬â€ this option is not available.'}
                </p>
              </button>
      ...
      >
        {cloneSource && (
          <div className="space-y-4">
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-3">
              <p className="text-sm text-blue-800 dark:text-blue-300">
                <strong>Cloning from:</strong> {cloneSource.course?.course_name || 'Unknown'} Ã¢â‚¬â€ {cloneSource.teacher?.name || 'Unknown'}
              </p>
              <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                Original: {cloneSource.day} Ã‚Â· {cloneSource.time || 'No time set'} Ã‚Â· {enrollmentCounts[cloneSource.session_id] || 0} students
              </p>
              <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
              ...
              >
                {cloning ? 'Cloning...' : `Ã°Å¸â€œâ€¹ Clone Session${cloneForm.copyEnrollments ? ' + Students' : ''}`}
              </Button>
            </div>
          </div>
        )}
